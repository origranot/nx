...
function addProjectIfExists(tree: Tree, configFilePath: string) {
  ...
  if (
    ...
    // Export is not a call expression object
    return;
  }
  ...
  let newProperties = [...firstArgument.properties];
  ...
  newProperties.push(newProjectsProperty);

  const newExportObject = ts.factory.createObjectLiteralExpression(
    newProperties,
    true
  );
